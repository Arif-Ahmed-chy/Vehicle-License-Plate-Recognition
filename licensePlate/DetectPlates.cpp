...
std::vector<PossiblePlate> detectPlatesInScene(cv::Mat &imgOriginalScene) {
	...
	cv::Mat imgMaxContrastGrayscaleScene;
	cv::Mat imgBlurredScene;
    ...
    preprocess(imgOriginalScene, imgGrayscaleScene, imgMaxContrastGrayscaleScene, imgBlurredScene, imgThreshScene);

#ifdef SHOW_STEPS
    cv::imshow("1a", imgGrayscaleScene);
	cv::imshow("1b", imgMaxContrastGrayscaleScene);
	cv::imshow("1c", imgBlurredScene);
	cv::imshow("1d", imgThreshScene);
#endif	
    std::vector<PossibleChar> vectorOfPossibleCharsInScene = findPossibleCharsInScene(imgThreshScene);

#ifdef SHOW_STEPS
    std::cout << "Valid Possible Characters = " << vectorOfPossibleCharsInScene.size() << std::endl;       
...
#ifdef SHOW_STEPS
    std::cout << "Groups of Matching Characters = " << vectorOfVectorsOfMatchingCharsInScene.size() << std::endl;       
